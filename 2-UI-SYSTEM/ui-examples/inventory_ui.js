...
exports.InventoryUI = exports.Button = void 0;
...
class InventoryItem extends Item {
     ...
     */
    constructor({ itemDescription, owner, }) {
        // Create a thumbnail from the item's texture asset
        const textureAsset = new core_1.TextureAsset(itemDescription.thumbnailId, itemDescription.thumbnailVersionId);
    ...
    constructThumbnailContent() {
        ...
        // Use Text
        const children = [(0, ui_1.Text)({
                text: "Use",
                style: this.contentStyle
            })];
        // Create a use button with currency icon and price
        const useButton = (0, ui_1.View)({
            children: Utils.WrapInFlex({
                children: children,
                style: {
                    height: '100%',
 ...
 */
class InventoryUI extends Panel {
    constructor() {
        super(...arguments);
        ...
        this.title = new ui_1.Binding("Inventory");
     ...
     */
    onContentBroadcastReceived({ id, metadata, items }) {
     ...
     * @param items - List of items
     */
    onContentReceived({ player, id, metadata, items }) {
     ...
     */
    onEntitlementsReceived({ player, id, entitlements }) {
        // Verify this component is the intended recipient
        if (!this.isRecipient(player, id))
            return;
