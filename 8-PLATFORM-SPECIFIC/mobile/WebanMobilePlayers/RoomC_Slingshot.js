...
class RoomC_Slingshot extends hz.Component {
    constructor() {
        super(...arguments);
        this.cooldown = false;
    }
    start() {
        ...
        const raycast = this.props.raycast;
        const ball = this.props.slingshotBall;
        const pouch = this.props.slingshotPouch;
        ...
        this.connectNetworkEvent(this.entity, sysEvents_1.sysEvents.OnFocusedInteractionInputEnded, (data) => {
            ...
            this.async.setTimeout(() => {
                if (ball)
                    ball.as(hz.PhysicalEntity).applyForce(deltaVector.mul(-1 * this.props.maxForce), hz.PhysicsForceMode.Impulse);
...
RoomC_Slingshot.propsDefinition = {
