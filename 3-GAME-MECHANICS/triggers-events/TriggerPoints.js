...
class TriggerPoints extends hz.Component {
    ...
    onPlayerEnterTrigger(player) {
        ...
        this.world.persistentStorage.setPlayerVariable(player, NEW_WORLD_POINTS, 
        // We get the current PPV score and we add 1 to it
        this.world.persistentStorage.getPlayerVariable(player, NEW_WORLD_POINTS) +
            1.0);
        const newTotal = this.world.persistentStorage.getPlayerVariable(player, NEW_WORLD_POINTS) +
            1.0;
        this.props.textGizmo?.as(hz.TextGizmo).text.set(`Points: ${newTotal}`);
        // update leaderboard with player's new PPV score
        this.world.leaderboards.setScoreForPlayer(NEW_WORLD_LEADERBOARD_NAME, player, this.world.persistentStorage.getPlayerVariable(player, NEW_WORLD_POINTS), false);
    }
}
TriggerPoints.propsDefinition = {
